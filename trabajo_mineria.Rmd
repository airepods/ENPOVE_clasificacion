...
=======
       caTools, yardstick, funModeling, DataExplorer, VIM,BiocManager, boruta)
>>>>>>> Stashed changes
...
PROBA.SVM <- predict(modelo2, newdata= data.test, type="prob")
head(PROBA.SVM)
PROBA.SVM <- PROBA.SVM[,2]

CLASE.SVM <- predict(modelo2,newdata = data.test) # 0.5
head(CLASE.SVM)
...
error <- mean(data.test$P701!=CLASE.SVM) ; error
...
library(caTools)
colAUC(PROBA.SVM,data.test$MORA60,plotROC = TRUE) -> auc_svm
abline(0, 1,col="red")
auc_svm
